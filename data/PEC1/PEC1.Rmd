...
Propón un proyecto completo de minería de datos. La organización de la respuesta tiene que coincidir en las fases típicas del ciclo de vida de un proyecto de minería de datos.
...
Conjunto de Datos de Accidentes (accident): Como ya sabemos por el analisis anterior tenemos que TWAY_ID2(vía de tránsito (2004) ) tiene 26,997 valores en blanco y como no es relvante para nuestro estudio no haremos nada con estos datos.
...
He simplificado las condiciones de luz en categorías como "Luz Diurna", "Oscuridad" y "Crepúsculo o Amanecer", junto con "Otras" para incluir casos no reportados o desconocidos.
...
# También se puede visualizar las contribuciones de las variables originales a los componentes principales
...
**Esta parte es extra porque queria probar los datos con el modelo randomForest pero me quede sin tiempo y estoy en el punto del balanceo de clases que es problema de este conjunto de datos**
...
# Importancia de las características (basado en los componentes principales, no directamente interpretable como las características originales)
...
Revisa el Equilibrio de Clases: Si una clase es mucho más frecuente que la otra, la alta precisión podría simplemente reflejar el desequilibrio de clases.
Validación Cruzada: Para asegurarte de que el modelo es robusto y generalizable, realiza una validación cruzada y verifica las métricas de desempeño en cada iteración.
Conjunto de Pruebas Independiente: Es vital tener un conjunto de datos de prueba que no haya sido visto por el modelo durante el entrenamiento para evaluar el rendimiento del modelo de manera objetiva.
...
# Corregir las clases:
En situaciones con un desequilibrio de clases tan pronunciado, la precisión no es la mejor métrica para evaluar el rendimiento del modelo, ya que puede ser engañosamente alta. En cambio, deberías considerar las siguientes métricas:
